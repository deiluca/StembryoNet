...
from constants import OUTDIR_ROOT, NR_CVS, NR_SPLITS, SBATCH_DIR, DATA_COLUMN, DATA_NAME, CHANNELS
...
for hour, _ in time_points:
    ...
    d = opj(OUTDIR_ROOT, f'resnet_{tname}_{DATA_NAME}')
    resd = opj(d, 'results')
    ...
    for cv in range(NR_CVS):
        for split in range(NR_SPLITS):
            ...
            outdir_name = f'resnet{model_depth}_{DATA_NAME}_{tname}'
