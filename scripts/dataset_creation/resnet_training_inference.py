...
for hour, t in time_points:
    ...
    outdir = opj(OUTDIR_ROOT, f'resnet_{tname}_{DATA_NAME}')
    ...
    for cv in range(NR_CVS):
        for split in range(NR_SPLITS):
            ...
            sbatchfilename = f'create_dataset_resnet_{tname}_{DATA_NAME}_cv{cv}_split{split}.sh'
...
save_imgs_selected_general_5cvs({cv}, train, val, test, {split}, outdir, t={t}, t_minus=0, t_col='plain', img_col='{DATA_COLUMN}', label_col='Score_str', id_col='embryo_id', channels={CHANNELS})
END'''
