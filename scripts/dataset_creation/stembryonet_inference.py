...
outdir = opj(OUTDIR_ROOT, f'stembryonet_inference_{DATA_NAME}')
...
for cv in range(NR_CVS):
    for split in range(NR_SPLITS):
        ...
        sbatchfilename = f'create_dataset_stembryonet_inference_{DATA_NAME}_cv{cv}_split{split}.sh'
...
save_imgs_selected_general_5cvs({cv}, train, val, test, {split}, outdir, t_minus=0, t_col='plain', img_col='{DATA_COLUMN}', label_col='Score_str', id_col='embryo_id', tmin={tmin}, tmax={tmax}, channels={CHANNELS}, savetrain=False)
END'''
