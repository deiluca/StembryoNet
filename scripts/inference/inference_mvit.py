...
from constants import OUTDIR_ROOT, NR_CVS, NR_SPLITS, SBATCH_DIR, DATA_NAME  # Import constants related to output directories and cross-validation splits
...
d = opj(OUTDIR_ROOT, f'mvit_{DATA_NAME}')  # Define output directory based on dataset name
...
for cv in range(NR_CVS):               # Loop over cross-validation splits
    for split in range(NR_SPLITS):     # Loop over dataset splits
        ...
        outdir_name = f'mvit_{DATA_NAME}'  # Define output directory name
        ...
        file_name = opj(SBATCH_DIR, outdir_name+f'_cv{cv}_split{split}.sh')
